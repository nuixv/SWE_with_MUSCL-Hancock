...
xmin = 0; xmax = 6;
ymin = 0; ymax = 6;
n = 60; %number of cells across box the x-axis
m = 60; %number of cells across box the y-axis
...
for i=1:n+4
    for j=1:m+4
        if((y(j) >= 0.5*sin(x(i))+2)) && ((y(j) < 0.5*sin(x(i))+3))
            z(i,j) = 0.0;
            h(i,j) = 0.0;
        else
            z(i,j) = 4;
...
for tstep=1:nt
    %boundary condition
      % x direction
       %left side
        for j=1:m+4               
          if((y(j) >= 0.5*sin(x(3))+2)) && ((y(j) < 0.5*sin(x(3))+3))
            h(1,j) = 2;
            h(2,j) = 2;   
            eta(1,j) = 2;
            eta(2,j) = 2;             
...
%--------------------------------------------------------------------------
    ...
    %open(v);
for nnn=1:nt
    ss = surf(xx,yy,hhh(:,:,nnn)');   %hhh = h ,hh = eta
    ss.EdgeColor = 'none';
    %ss.FaceColor = 'b';
    hold on
    zz = surf(xx,yy,z(:,:)');
